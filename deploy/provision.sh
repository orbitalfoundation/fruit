#!/usr/bin/env bash
# One-time provisioning for the Fruit VM on exe.dev. Idempotent — safe to re-run;
...
# Usage: deploy/provision.sh [vm-name]      (default: fruit)
set -euo pipefail
...
$SSH "$DEST" "sudo systemctl enable --now docker >/dev/null 2>&1; \
  docker rm -f fruit 2>/dev/null || true; \
  docker run -d --name fruit --restart unless-stopped \
    -p $PORT:80 \
